...
int BST::getHeight(BinaryNode* t)
{
	...
	{
		int currentHeight = 1;
		int leftHeight = currentHeight + getHeight(t->left);
		int rightHeight = currentHeight + getHeight(t->right);
...
bool BST::isBalanced(BinaryNode* t)
{
	if (t != NULL)
	{
		if (isBalanced(t->left) == false)
		{
			return(false);
		}
		else if (isBalanced(t->right) == false)
		{
			return(false);
		}
		else
